...
export function useToast() {
  ...
  const toast = useCallback(({ variant, text, duration = 3000 }: ToastConfig) => {
    const id = Date.now()
    ...
    <div className="fixed top-6 right-6 z-[100] flex flex-col gap-2 w-full max-w-sm pointer-events-none">
      {toasts.map(t => {
        ...
        return (
          ...
          >
            <span className={`mt-0.5 flex-shrink-0 ${style.icon}`}>{ICONS[t.variant]}</span>
            <p className={`flex-1 text-sm font-medium leading-snug ${style.text}`}>{t.text}</p>
            <button
